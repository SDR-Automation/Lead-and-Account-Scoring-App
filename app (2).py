...
def get_db():
    ...
    if mc is None:
        return None
    return mc["ebryx_lead_intelligence"]


def save_run(file_name: str, df_input: pd.DataFrame, df_results: pd.DataFrame):
    db = get_db()
    if db is None:
        return None
    ...
    doc = {
        ...
        "timestamp"      : now,
        "file_name"      : file_name,
        "total_companies": len(df_results),
        "summary"        : {
            "very_high": int(len(df_results[df_results["index_score"] >= 40])),
            "high"     : int(len(df_results[(df_results["index_score"] >= 30) & (df_results["index_score"] < 40)])),
            "medium"   : int(len(df_results[(df_results["index_score"] >= 20) & (df_results["index_score"] < 30)])),
            "low"      : int(len(df_results[df_results["index_score"] < 20])),
        },
        "input_records"  : df_input.to_dict(orient="records"),
        "scored_records" : df_results.to_dict(orient="records"),
    }
...
def load_all_runs():
    ...
    cursor = db["runs"].find(
        {},
        {"run_id": 1, "timestamp": 1, "file_name": 1,
         "total_companies": 1, "summary": 1, "_id": 1}
    ).sort("timestamp", -1).limit(50)
    return list(cursor)


def load_run_by_id(run_id: str):
    db = get_db()
    if db is None:
        return None
...
[data-testid="stSidebar"] * { color: var(--grey-1) !important; }
...
.db-badge.connected    { background: rgba(0,191,89,0.08); border: 1px solid rgba(0,191,89,0.25); color: #00BF59; }
.db-badge.disconnected { background: rgba(196,26,26,0.08); border: 1px solid rgba(196,26,26,0.25); color: #FF4040; }
.db-dot { width: 6px; height: 6px; border-radius: 50%; }
.db-dot.on  { background: #00BF59; box-shadow: 0 0 5px rgba(0,191,89,0.5); }
.db-dot.off { background: #FF4040; }

.run-card {
    background: var(--black-3); border: 1px solid var(--border);
    border-radius: 5px; padding: 12px 14px; margin-bottom: 8px;
}
.run-card.active { border-color: var(--red); border-left: 2px solid var(--red); }
.run-ts   { font-family: 'DM Mono', monospace; font-size: 0.6rem; color: var(--grey-3); letter-spacing: 0.1em; margin-bottom: 4px; }
.run-file { font-size: 0.78rem; color: var(--grey-1); font-weight: 500; margin-bottom: 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.run-pills { display: flex; gap: 5px; flex-wrap: wrap; }
.rpill { font-family: 'DM Mono', monospace; font-size: 0.58rem; padding: 2px 7px; border-radius: 3px; }
.rpill.vh  { background: rgba(255,64,64,0.15);  color: #FF4040; border: 1px solid rgba(255,64,64,0.3); }
.rpill.hi  { background: rgba(255,140,0,0.15);  color: #FF8C00; border: 1px solid rgba(255,140,0,0.3); }
.rpill.med { background: rgba(160,160,160,0.1); color: #A0A0A0; border: 1px solid rgba(160,160,160,0.2); }
.rpill.lo  { background: rgba(90,90,90,0.1);    color: #5a5a5a; border: 1px solid rgba(90,90,90,0.2); }
...
.topnav-brand { font-family: 'Bebas Neue', sans-serif; font-size: 1.4rem; letter-spacing: 0.12em; color: var(--white); display: flex; align-items: center; gap: 10px; }
.topnav-dot { width: 10px; height: 10px; background: var(--red); border-radius: 50%; display: inline-block; box-shadow: 0 0 8px rgba(196,26,26,0.6); }
.topnav-divider { width: 1px; height: 24px; background: var(--border); }
.topnav-tag { font-family: 'DM Mono', monospace; font-size: 0.68rem; color: var(--grey-3); letter-spacing: 0.1em; text-transform: uppercase; border: 1px solid var(--border); padding: 4px 10px; border-radius: 4px; }
.topnav-right { display: flex; align-items: center; gap: 8px; font-family: 'DM Mono', monospace; font-size: 0.68rem; color: var(--red-bright); letter-spacing: 0.1em; }
.live-dot { width: 7px; height: 7px; background: var(--red-bright); border-radius: 50%; animation: blink 1.5s infinite; }
@keyframes blink { 0%,100%{opacity:1} 50%{opacity:0.3} }

.hero {
    background: var(--black); padding: 60px 48px 48px;
    border-bottom: 1px solid var(--border); position: relative; overflow: hidden;
    text-align: center; display: flex; flex-direction: column;
    align-items: center; justify-content: center;
}
.hero::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, var(--red), transparent); }
.hero-label { font-family: 'DM Mono', monospace; font-size: 0.68rem; color: var(--red-bright); letter-spacing: 0.2em; text-transform: uppercase; margin-bottom: 16px; display: flex; align-items: center; justify-content: center; gap: 10px; }
...
.hero-h1 { font-family: 'Bebas Neue', sans-serif; font-size: 5rem; line-height: 0.95; letter-spacing: 0.03em; color: var(--white); margin-bottom: 20px; text-align: center; }
.hero-h1 .red { color: var(--red-bright); }
.hero-p { font-size: 0.95rem; color: var(--grey-2); max-width: 500px; line-height: 1.8; font-weight: 300; text-align: center; margin: 0 auto; }
.hero-bg-text { position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%); font-family: 'Bebas Neue', sans-serif; font-size: 14rem; color: rgba(196,26,26,0.03); letter-spacing: 0.05em; user-select: none; pointer-events: none; line-height: 1; white-space: nowrap; }

.sec-hdr { font-family: 'DM Mono', monospace; font-size: 0.62rem; font-weight: 500; color: var(--grey-3); letter-spacing: 0.2em; text-transform: uppercase; margin-bottom: 20px; display: flex; align-items: center; gap: 12px; }
.sec-hdr::after { content: ''; flex: 1; height: 1px; background: var(--border); }

.req-card { background: var(--black-3); border: 1px solid var(--border); border-top: 2px solid var(--red); border-radius: 6px; padding: 16px 18px; margin-bottom: 20px; }
.req-card-title { font-family: 'DM Mono', monospace; font-size: 0.62rem; color: var(--grey-3); letter-spacing: 0.15em; text-transform: uppercase; margin-bottom: 12px; }
.col-pill { display: inline-block; background: var(--red-light); border: 1px solid var(--red-border); color: var(--red-bright); padding: 3px 10px; border-radius: 3px; font-family: 'DM Mono', monospace; font-size: 0.72rem; margin: 2px; letter-spacing: 0.05em; }

.audit-card { background: var(--black-3); border: 1px solid var(--border); border-radius: 6px; padding: 16px 18px; margin-bottom: 12px; }
.audit-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; }
.audit-key { font-family: 'DM Mono', monospace; font-size: 0.6rem; color: var(--grey-3); letter-spacing: 0.12em; text-transform: uppercase; }
.audit-val { font-family: 'DM Mono', monospace; font-size: 0.7rem; color: var(--grey-1); }

.metrics-row { display: grid; grid-template-columns: repeat(5, 1fr); gap: 10px; margin-bottom: 24px; }
.met-card { background: var(--black-2); border: 1px solid var(--border); border-radius: 6px; padding: 16px 12px; text-align: center; position: relative; }
.met-card::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 2px; border-radius: 0 0 6px 6px; }
...
.rationale-box { background: var(--black-3); border: 1px solid var(--border); border-left: 2px solid var(--grey-4); border-radius: 4px; padding: 14px 16px; font-size: 0.8rem; color: var(--grey-2); line-height: 1.7; }
.action-box { background: var(--red-light); border: 1px solid var(--red-border); border-left: 2px solid var(--red); border-radius: 4px; padding: 14px 16px; font-size: 0.8rem; color: var(--grey-1); line-height: 1.7; font-weight: 500; }
...
.stButton > button { background: var(--red) !important; color: var(--white) !important; font-family: 'Bebas Neue', sans-serif !important; font-size: 1rem !important; letter-spacing: 0.15em !important; border: none !important; border-radius: 4px !important; padding: 14px 24px !important; width: 100% !important; text-transform: uppercase !important; transition: all 0.15s ease !important; }
...
.stDownloadButton > button { background: var(--black-3) !important; color: var(--grey-1) !important; font-family: 'Bebas Neue', sans-serif !important; font-size: 1rem !important; letter-spacing: 0.15em !important; border: 1px solid var(--border-red) !important; border-radius: 4px !important; padding: 14px 24px !important; width: 100% !important; text-transform: uppercase !important; transition: all 0.15s ease !important; }
...
def score_all(df, prog, status, chunk_size=5):
    ...
    for i in range(0, total, chunk_size):
        ...
        status.markdown(
            f"<p style='font-family:DM Mono,monospace;font-size:0.75rem;color:#5a5a5a;'>"
            f"> PROCESSING {i+1}–{end} / {total}</p>",
            unsafe_allow_html=True
        )
        prog.progress(pct)
        try:
    ...
    status.markdown(
        "<p style='font-family:DM Mono,monospace;font-size:0.75rem;color:#FF4040;'>"
        "> ANALYSIS COMPLETE</p>",
        unsafe_allow_html=True
    )
    prog.progress(100)
...
def export_excel(df):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        col_widths = {1:22,2:28,3:35,4:18,5:16,6:14,7:16,8:18,9:12,10:14,11:16,12:20,13:20,14:14,15:16,16:30,17:45}
...
def render_sidebar():
    with st.sidebar:
        db_connected = get_mongo_client() is not None

        if db_connected:
            st.markdown('<div class="db-badge connected"><span class="db-dot on"></span>ATLAS CONNECTED</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="db-badge disconnected"><span class="db-dot off"></span>NO DB CONNECTION</div>', unsafe_allow_html=True)
            st.markdown(
                "<p style='font-family:DM Mono,monospace;font-size:0.62rem;color:#5a5a5a;line-height:1.8;'>"
                "Set <b style='color:#A0A0A0'>MONGO_URI</b> in secrets.toml to enable logging.</p>",
                unsafe_allow_html=True
            )

        st.markdown('<div class="sec-hdr" style="margin-top:12px;">Audit Log</div>', unsafe_allow_html=True)

        if not db_connected:
            st.markdown("<p style='font-family:DM Mono,monospace;font-size:0.6rem;color:#3a3a3a;'>No runs logged.</p>", unsafe_allow_html=True)
            return
        ...
        if not runs:
            st.markdown(
                "<p style='font-family:DM Mono,monospace;font-size:0.6rem;color:#3a3a3a;'>"
                "No runs yet. Score your first batch to begin logging.</p>",
                unsafe_allow_html=True
            )
            return

        active_run = st.session_state.get("loaded_run_id")

        for run in runs:
            run_id  = run["run_id"]
            ts      = run["timestamp"].strftime("%Y-%m-%d  %H:%M UTC")
            fname   = run.get("file_name", "unknown")
            summary = run.get("summary", {})
            total   = run.get("total_companies", 0)
            active  = "active" if run_id == active_run else ""

            pills = ""
            if summary.get("very_high", 0): pills += f'<span class="rpill vh">{summary["very_high"]} VH</span>'
            if summary.get("high", 0):      pills += f'<span class="rpill hi">{summary["high"]} HI</span>'
            if summary.get("medium", 0):    pills += f'<span class="rpill med">{summary["medium"]} MED</span>'
            if summary.get("low", 0):       pills += f'<span class="rpill lo">{summary["low"]} LO</span>'

            st.markdown(f"""
            <div class="run-card {active}">
                <div class="run-ts">{ts}</div>
                <div class="run-file" title="{fname}">{fname}</div>
                <div style="font-family:'DM Mono',monospace;font-size:0.58rem;color:#3a3a3a;margin-bottom:6px;">{total} companies</div>
            ...
            with col_load:
                if st.button("Load", key=f"load_{run_id}"):
                    doc = load_run_by_id(run_id)
                    if doc and doc.get("scored_records"):
                        st.session_state["df_results"]      = pd.DataFrame(doc["scored_records"])
                        st.session_state["loaded_run_id"]   = run_id
                        st.session_state["loaded_run_meta"] = {
                            "run_id"   : run_id,
                            "timestamp": ts,
                            "file_name": fname,
                            "total"    : total,
            ...
            with col_del:
                if st.button("✕", key=f"del_{run_id}"):
                    ...
                    if st.session_state.get("loaded_run_id") == run_id:
                        ...
                        st.session_state.pop("loaded_run_meta", None)
                    st.rerun()
...
def main():
    ...
    db_connected = get_mongo_client() is not None
    db_label     = "ATLAS CONNECTED" if db_connected else "DB OFFLINE"
    db_color     = "#00BF59" if db_connected else "#5a5a5a"

    st.markdown(f"""
    <div class="topnav">
        <div class="topnav-left">
            <div class="topnav-brand"><span class="topnav-dot"></span>EBRYX</div>
            <div class="topnav-divider"></div>
            <div class="topnav-tag">Lead Intelligence Platform</div>
        </div>
        <div class="topnav-right" style="gap:18px;">
            <span style="color:{db_color};font-size:0.62rem;letter-spacing:0.1em;">{db_label}</span>
            <div style="width:1px;height:16px;background:#2a2a2a;"></div>
    ...
    <div class="hero">
        ...
        <div class="hero-h1">SCORE &amp;<br><span class="red">PRIORITIZE</span><br>YOUR LEADS</div>
        <p class="hero-p">Upload your prospect list. Our AI engine researches, scores, and ranks every
        company across 5 strategic dimensions — every run is logged to MongoDB Atlas.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_l:
        ...
        if "loaded_run_meta" in st.session_state:
            meta = st.session_state["loaded_run_meta"]
            s    = meta["summary"]
            st.markdown(f"""
            <div style="margin-top:24px;">
                <div class="sec-hdr">Loaded Run</div>
                <div class="audit-card">
                    <div class="audit-row"><span class="audit-key">Run ID</span><span class="audit-val">{meta['run_id']}</span></div>
                    <div class="audit-row"><span class="audit-key">Timestamp</span><span class="audit-val">{meta['timestamp']}</span></div>
                    <div class="audit-row"><span class="audit-key">File</span><span class="audit-val" style="max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{meta['file_name']}</span></div>
                    <div class="audit-row"><span class="audit-key">Companies</span><span class="audit-val">{meta['total']}</span></div>
                    <div style="margin-top:8px;display:flex;gap:5px;flex-wrap:wrap;">
                        <span class="rpill vh">{s.get('very_high',0)} VH</span>
                        <span class="rpill hi">{s.get('high',0)} HI</span>
                        <span class="rpill med">{s.get('medium',0)} MED</span>
                        <span class="rpill lo">{s.get('low',0)} LO</span>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with col_r:
        ...
        if run_button and df_input is not None:
            ...
            if not df_res.empty and db_connected:
                try:
                    run_id, oid = save_run(uploaded.name, df_input, df_res)
                    st.success(f"✓ Saved to Atlas  ·  Run ID: {run_id}")
                except Exception as e:
                    st.warning(f"Scoring complete, but MongoDB save failed: {e}")
            elif not db_connected:
                st.warning("MongoDB not connected — results will not be persisted.")
        ...
        if "df_results" not in st.session_state:
            ...
            <div class="empty-st">
                ...
                <div class="empty-sub">Upload a file and run the scoring engine, or load a past run from the sidebar.</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            for _, row in df_d.iterrows():
                ...
                with st.expander(f"  {company}   ·   {score}/50   ·   {priority}"):
                    ...
                    for col_obj, lbl, val in [
                        ...
                        with col_obj:
                            st.markdown(
                                f'<div class="detail-label">{lbl}</div>'
                    ...
                    for lbl, key in [
                        ...
                        bars += (
                            ...
                            f'<div class="sb-track"><div class="sb-fill" style="width:{pct}%;background:{c};"></div></div>'
                            f'<span class="sb-val" style="color:{c}">{v}</span>'
                            f'</div>'
                        )

                    st.markdown(
                        f'<div style="background:var(--black-3);border:1px solid var(--border);'
                        f'border-top:1px solid var(--border-red);border-radius:4px;padding:16px 20px;'
                        f'margin-bottom:12px;">{bars}</div>',
                        unsafe_allow_html=True
                    )
                    ...
                    with r1:
                        st.markdown(
                            ...
                            f'<b style="font-size:0.62rem;color:#3a3a3a;letter-spacing:0.15em;'
                            f'text-transform:uppercase;font-family:DM Mono,monospace;">Rationale</b>'
                            f'<br><br>{row.get("rationale","—")}</div>',
                            unsafe_allow_html=True
                        )
                    with r2:
                        st.markdown(
                            f'<div class="action-box">'
                            f'<b style="font-size:0.62rem;color:#C41A1A;letter-spacing:0.15em;'
                            f'text-transform:uppercase;font-family:DM Mono,monospace;">Sales Action</b>'
                            f'<br><br>{row.get("sales_action","—")}</div>',
                            unsafe_allow_html=True
                        )
